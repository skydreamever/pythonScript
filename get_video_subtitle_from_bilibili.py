...
APPKEY = '85eb6835b0a1034e'  # 现在这个key和secret应该可以不用了
...
def biligrab(url, *, debug=False, verbose=False, media=None, cookie=None, quality=None, source=None, keep_fps=False, mpvflags=[], d2aflags={}):
    ...
    def convert_comments(cid, video_size):
        ...
        for i, j in ((('stage_width', 'stage_height', 'reserve_blank'), int), (('font_size', 'text_opacity', 'comment_duration'), float)):
            for k in i:
                if k in d2aflags:#根据输入的一些danmaku2ass处理标志来处理，否则默认
    ...
    aid, pid = parse_url(url)#aid av的code

    logging.info('Loading video info...')
    video_metadata = fetch_video_metadata(aid, pid) #根据aid获得cid

    logging.info('Got video cid: %s' % video_metadata['cid'])#返回的json数据中又cid选项
...
def check_env(debug=False):
    ...
    try:
        import danmaku2ass
        #python3脚本，用来将bilibili的comment的xml文件转换为ass文件的，项目地址https://github.com/m13253/danmaku2ass
    ...
    try:
        ...
        '''
        用来查看mpv信息的，据说mpv的性能要比mplayerx好，mac安装方式
        brew tap mpv-player/mpv
        brew install mpv
        '''
        mpv_output = mpv_process.communicate()[0].decode('utf-8', 'replace').splitlines()#通信，实际上是获得输出结果
    ...
    try:
        mpv_process = subprocess.Popen(('mpv', '--vf', 'lavfi=help'), stdout=subprocess.DEVNULL)#这里表示结果重定向到/dev/null
        #lavfi是虚拟输入设备，使得输入不一定是本地磁盘文件，也可以是流（就是流媒体）
        mpv_process.wait()#父线程等待子线程执行完毕
...
def main():
    ...
    parser.add_argument('-c', '--cookie', help='Import Cookie at bilibili.com, type document.cookie at JavaScript console to acquire it, step:chrome->Inspect Element->Console->type document.cookie->copy and paste')
